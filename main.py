...
def add_ways():
    ...
    if form.validate_on_submit():
        ...
        with open(f'static/img/chart/{ways.id}.jpg', 'wb') as f:
            f.write(request.files['photo'].read())

        return redirect('/')
    return render_template('ways.html', title='Добавить памятное место', form=form)
...
def edit_ways(id):
    ...
    if form.validate_on_submit():
        ...
        if ways:
            ...
            abort(404)
    return render_template('ways.html', title='Редактирование', form=form)
